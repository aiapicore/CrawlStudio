...
async def run_backend_comparison():
    ...
    for format_type in formats:
        ...
        for name, backend in backends.items():
            try:
                ...
                print(f"{name:12} | {result.execution_time:6.2f}s | {content_length:8,} chars | Cache: {result.cache_hit}")
    ...
    for name, stats in performance_results.items():
        if "error" in stats:
            ...
            print(f"{name:12} | Avg: {stats['avg']:.2f}s | Min: {stats['min']:.2f}s | Max: {stats['max']:.2f}s")
